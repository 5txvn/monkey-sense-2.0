...
$("#requireenter").change(() => {
  if (document.getElementById("requireenter").checked) {
    $("#wrong").show();
  } else {
    $("#wrong").hide();
  }
...
function updateData() {
  ...
  $("#questioncount").text(`Questions Answered: ${questionCount}`)
...
function handleMessages(arr) {
  for (const message of arr) {
    if (message.type === "question") {
      ...
      $("#roundNumber").text(`Question: ${message.data[0]}`)
      $("#question").text(message.data[1])
      $("#inputbox").val("");
      ...
      if (message.tag === "start") {
        ...
        if (message.tag === "answer") {
          ...
          if (shouldRequireEnter()) {
            ...
            wrong++; streak = 0;
            $("#wrong").html(wrong + '<i class="fa fa-times pl-1"></i>'); $("#streak").html(streak + '<i class="fa fa-dashboard pl-2"></i>');
          }
